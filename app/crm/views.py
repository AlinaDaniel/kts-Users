...
class AddUserView(View):
    ...
    async def post(self):
        # data = await self.request.json() до валидации
...
class ListUsersView(View):
    ...
    async def get(self):
        ...
        if not check_basic_auth(self.request.headers["Authorization"],username=self.request.app
                                .config.username,password=self.request.app.config.password):
            raise HTTPForbidden
...
class GetUserView(View):
    ...
    async def get(self):
        ...
        if user:
            # return json_response(data={"user": {"email": user.email, "id": str(user.id_)}}) до авторизации
